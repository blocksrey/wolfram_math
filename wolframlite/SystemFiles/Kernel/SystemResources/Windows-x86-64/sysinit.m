...
System`$CharacterEncodings = DeleteCases[StringTake[#, {1, -3}] & /@ FileNames[ "*.m"],"Unicode" | "ISO10646-1"];
...
(* Load all the non-autoload symbols defined in top-level and protected them.
 * The startup build saved the names in System`Private`nonAutoLoads *)
Get[FileNameJoin[{$SystemFileDir,"NonAutoLoads.mx"}]]
SetAttributes[#,{Protected,ReadProtected}]& /@ nonAutoLoads
...
Experimental`ValueFunction[ System`$SSHAuthentication] := (System`RemoteConnect; True)
...
QuantityUnits`Private`$QuantityUnitsAutoloads = Hold[
    ...
    QuantityUnits`ToQuantityShortString, QuantityUnits`$AutomaticUnitTimes,
...
Package`DeclareLoad[
        {System`GenerateDocument, System`NotebookTemplate, NotebookTemplating`CreateTemplateNotebook,
            NotebookTemplating`ClearTemplateNotebook, NotebookTemplating`TemplateNotebookQ,NotebookTemplating`Utilities`GenerateDocumentInterface},
        "NotebookTemplating`",
        Package`HiddenImport -> True
    ]
